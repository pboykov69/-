...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Ложь, ВысотаЗаголовка = 0, ТолькоЗаголовок = Ложь, ОчищатьТабличныйДокумент = Истина) Экспорт
	...
	Для каждого СтрокаТаблицаСотрудников Из ТаблицаСотрудников Цикл
		...
		ТаблОтбора.Сортировать("Период Убыв");
		Для каждого СтрокаТаблОтбора Из ТаблОтбора Цикл
			...
			КонецЕсли; 
		КонецЦикла; 
		ТаблОтбора.Сортировать("Период Возр");
		Для каждого СтрокаТаблОтбора Из ТаблОтбора Цикл
			Если ТипЗнч(СтрокаТаблОтбора.Регистратор)=Тип("ДокументСсылка.ПриемНаРаботуВОрганизацию") Тогда	
				ОбластьШапка.Параметры.ПодразделениеОрганизации = СокрЛП(СтрокаТаблОтбора.ПодразделениеОрганизации);
		...
		ДанныеФизЛица = ОбщегоНазначения.ДанныеФизЛица(СтрокаТаблицаСотрудников.Организация, СтрокаТаблицаСотрудников.Физлицо, ?(ЗначениеЗаполнено(ДатаКон), ДатаКон, РабочаяДата), Ложь);
		...
		ЗаполнитьСрокиПолезногоИспользования(ТаблОтбора);
		
		ТаблОтбора.Свернуть("Номенклатура, Организация, Физлицо, СрокПолезногоИспользования, Количество");
		Для каждого СтрокаТаблОтбора Из ТаблОтбора Цикл
			ОбластьСтрокаНорм = Макет.ПолучитьОбласть("СтрокаЛицевойСтороны");
			ЗаполнитьЗначенияСвойств(ОбластьСтрокаНорм.Параметры,СтрокаТаблОтбора);
			Если СтрокаТаблОтбора.СрокПолезногоИспользования = 0 Тогда
				ОбластьСтрокаНорм.Параметры.СрокПолезногоИспользования = "бессроч.";
			Иначе	
				ОбластьСтрокаНорм.Параметры.СрокПолезногоИспользования = Окр(12/СтрокаТаблОтбора.СрокПолезногоИспользования,2)*СтрокаТаблОтбора.Количество;
			КонецЕсли; 
		...
		ДанныеФизЛица = ОбщегоНазначения.ДанныеФизЛица(СтрокаТаблицаСотрудников.Организация, ОтветственноеЛицо.Физлицо, ?(ЗначениеЗаполнено(ДатаКон), ДатаКон, РабочаяДата), Истина);
		...
		Для каждого СтрокаТаблОтбора Из ТаблОтбора Цикл
			...
			Если ПодставлятьНомераВедомостей Тогда
				Если ЗначениеЗаполнено(СтрокаТаблОтбора.ДокументВыдачи) Тогда
					ОбластьСтрокаТЧ.Параметры.ДокументВыдачи = СтрокаТаблОтбора.ДокументВыдачи.Метаданные().Синоним +
						" №" + СокрЛП(ОбщегоНазначения.ПолучитьНомерНаПечать(СтрокаТаблОтбора.ДокументВыдачи)) +
						" от " + Формат(СтрокаТаблОтбора.ДокументВыдачи.Дата,"ДФ=dd.MM.yyyy");
				КонецЕсли; 
				Если ЗначениеЗаполнено(СтрокаТаблОтбора.ДокументВыбытия) Тогда
					ОбластьСтрокаТЧ.Параметры.ДокументВыбытия = СтрокаТаблОтбора.ДокументВыбытия.Метаданные().Синоним +
						" №" + СокрЛП(ОбщегоНазначения.ПолучитьНомерНаПечать(СтрокаТаблОтбора.ДокументВыбытия)) +
						" от " + Формат(СтрокаТаблОтбора.ДокументВыбытия.Дата,"ДФ=dd.MM.yyyy");
				КонецЕсли; 
			КонецЕсли; 
...
Функция ПолучитьТаблицуПараметровСотрудников()
	...
	ЗапросСПО.УстановитьПараметр("ДатаОтчета", КонецДня( ?(ЗначениеЗаполнено(ДатаКон), ДатаКон, РабочаяДата) ) );
	...
	|ГДЕ
	|	РаботникиОрганизаций.Период <= &ДатаОтчета
	|	И РаботникиОрганизаций.Организация = &Организация
	|	И РаботникиОрганизаций.Сотрудник.ФизЛицо = &СотрудникФизЛицо
	|
	|УПОРЯДОЧИТЬ ПО
	|	Период УБЫВ";
	...
	Иначе
		ЗапросСПО.Текст = СтрЗаменить(ЗапросСПО.Текст, "И РаботникиОрганизаций.Организация = &Организация", "");		 
	КонецЕсли;
	...
	Если ОтборРаботника Тогда
		...
		ЗапросСПО.Текст = СтрЗаменить(ЗапросСПО.Текст, ".Сотрудник.ФизЛицо = &СотрудникФизЛицо", ".Сотрудник.ФизЛицо " + ВидСравненияРаботникаСтрокой + " (&СотрудникФизЛицо)");
		
		ЗапросСПО.УстановитьПараметр("СотрудникФизЛицо", Работник);
		
	Иначе
		ЗапросСПО.Текст = СтрЗаменить(ЗапросСПО.Текст, "И РаботникиОрганизаций.Сотрудник.ФизЛицо = &СотрудникФизЛицо", "");
	КонецЕсли;
...
Функция ПолучитьТаблицуВыданнойСпецодежды()
	...
	ЗапросСПО.УстановитьПараметр("ДатаОтчета", новый граница(КонецДня( ?(ЗначениеЗаполнено(ДатаКон), ДатаКон, РабочаяДата) ), ВидГраницы.Включая));
	ЗапросСПО.УстановитьПараметр("ДатаПриема", НачалоДня( ?(ЗначениеЗаполнено(ДатаКон), ДатаКон, РабочаяДата) ));
	ЗапросСПО.УстановитьПараметр("ДатаУвольнения", КонецДня( ?(ЗначениеЗаполнено(ДатаКон), ДатаКон, РабочаяДата) ));
	ЗапросСПО.УстановитьПараметр("СвойствоГОСТ", СвойствоГОСТ);
	ЗапросСПО.УстановитьПараметр("НачалоПериода", НачалоДня(ДатаНач));
	ЗапросСПО.УстановитьПараметр("КонецПериода", КонецДня( ?(ЗначениеЗаполнено(ДатаКон), ДатаКон, РабочаяДата) ));
	...
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ПередачаМатериаловВЭксплуатациюМатериалы.Ссылка.Дата >= &НачалоПериода
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА &КонецПериода = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ &КонецПериода >= ПередачаМатериаловВЭксплуатациюМатериалы.Ссылка.Дата
	|		КОНЕЦ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|			КОГДА &НачалоПериода = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ВводНачальныхОстатковМатериаловВЭксплуатацииМатериалы.Ссылка.Дата >= &НачалоПериода
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА &КонецПериода = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ &КонецПериода >= ВводНачальныхОстатковМатериаловВЭксплуатацииМатериалы.Ссылка.Дата
	|		КОНЕЦ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|			КОГДА &НачалоПериода = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ПеремещениеМатериаловВЭксплуатацииМатериалы.Ссылка.Дата >= &НачалоПериода
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА &КонецПериода = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ &КонецПериода >= ПеремещениеМатериаловВЭксплуатацииМатериалы.Ссылка.Дата
	|		КОНЕЦ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|			КОГДА &НачалоПериода = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ СписаниеМатериаловИзЭксплуатацииМатериалы.Ссылка.Дата >= &НачалоПериода
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА &КонецПериода = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ &КонецПериода >= СписаниеМатериаловИзЭксплуатацииМатериалы.Ссылка.Дата
	|		КОНЕЦ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|			КОГДА &НачалоПериода = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ВозвратМатериаловИзЭксплуатацииМатериалы.Ссылка.Дата >= &НачалоПериода
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА &КонецПериода = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ &КонецПериода >= ВозвратМатериаловИзЭксплуатацииМатериалы.Ссылка.Дата
	|		КОНЕЦ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|ГДЕ
	|	ДокументыПеремещенияПоступление.ДокументВыдачи <> ЗНАЧЕНИЕ(Документ.ПередачаМатериаловВЭксплуатацию.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	ДокументыВводаОстатков.ДокументВыдачи <> ЗНАЧЕНИЕ(Документ.ПередачаМатериаловВЭксплуатацию.ПустаяСсылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция РассчитатьПроцентИзноса(НазначениеИспользования, ДатаВыдачи, ДатаСписания)
	...
	Если Не ЗначениеЗаполнено(ДатаВыдачи) Или Не ЗначениеЗаполнено(ДатаСписания) Тогда
		Возврат 0;
	КонецЕсли;
...
Процедура ЗаполнитьСрокиПолезногоИспользования(Таблица)
	
	Таблица.Колонки.Добавить("СрокПолезногоИспользования");
	Таблица.Колонки.Добавить("Количество");
	Для Каждого СтрТаб Из Таблица Цикл
		СтрТаб.СрокПолезногоИспользования = СтрТаб.НазначениеИспользования.СрокПолезногоИспользования;
		СтрТаб.Количество = СтрТаб.НазначениеИспользования.Количество;
	КонецЦикла;
